...
function restartGame() {
  field.cells.forEach((cells) =>
    cells.forEach((cell) => {
      if (!cell.isEmpty()) cell.unlinkTileAndDelete();
    })
  );
  startGame();
}
...
function startGame() {
  ...
  createVertivalRoads(field, FIELD_WIDTH, FIELD_HEIGHT);
  createHorizontalRoads(field, FIELD_WIDTH, FIELD_HEIGHT);
  spawnTile();
...
function heroIsNear(masEnemy, i) {
  if (
    masEnemy[i].y - 1 != -1 &&
    field.cells[masEnemy[i].y - 1][masEnemy[i].x].isType() == "Hero"
  ) {
    return field.cells[masEnemy[i].y - 1][masEnemy[i].x];
  } else if (
    masEnemy[i].y + 1 != FIELD_HEIGHT &&
    field.cells[masEnemy[i].y + 1][masEnemy[i].x].isType() == "Hero"
  ) {
    return field.cells[masEnemy[i].y + 1][masEnemy[i].x];
  } else if (
    masEnemy[i].x - 1 != -1 &&
    field.cells[masEnemy[i].y][masEnemy[i].x - 1].isType() == "Hero"
  ) {
    return field.cells[masEnemy[i].y][masEnemy[i].x - 1];
  } else if (
    masEnemy[i].x + 1 != FIELD_WIDTH &&
    field.cells[masEnemy[i].y][masEnemy[i].x + 1].isType() == "Hero"
  ) {
    return field.cells[masEnemy[i].y][masEnemy[i].x + 1];
  }
...
function moveEnemy() {
  ...
  field.cells.map((cells) =>
    cells.map((cell) => {
      if (cell.isType() == "Enemy") masEnemy.push(cell);
    })
  );
  ...
  for (let i = 0; i < masEnemy.length; i++) {
    ...
    while (flag && !heroIsNear(masEnemy, i)) {
      ...
      if (
        derection == "up" &&
        masEnemy[i].y - 1 != -1 &&
        field.cells[masEnemy[i].y - 1][masEnemy[i].x].isType() != "Wall" &&
        field.cells[masEnemy[i].y - 1][masEnemy[i].x].isType() != "Hero" &&
        field.cells[masEnemy[i].y - 1][masEnemy[i].x].isType() != "Enemy" &&
        field.cells[masEnemy[i].y - 1][masEnemy[i].x].isType() != "Heal" &&
        field.cells[masEnemy[i].y - 1][masEnemy[i].x].isType() != "Sword"
      ) {
        field.cells[masEnemy[i].y - 1][masEnemy[i].x].linkTile(
          enemyTile.linkedTile
        );
        ...
        derection == "left" &&
        masEnemy[i].x - 1 != -1 &&
        field.cells[masEnemy[i].y][masEnemy[i].x - 1].isType() != "Wall" &&
        field.cells[masEnemy[i].y][masEnemy[i].x - 1].isType() != "Hero" &&
        field.cells[masEnemy[i].y][masEnemy[i].x - 1].isType() != "Enemy" &&
        field.cells[masEnemy[i].y][masEnemy[i].x - 1].isType() != "Heal" &&
        field.cells[masEnemy[i].y][masEnemy[i].x - 1].isType() != "Sword"
      ) {
        field.cells[masEnemy[i].y][masEnemy[i].x - 1].linkTile(
          enemyTile.linkedTile
        );
        enemyTile.unlinkTile();
        flag = false;
    ...
    for (let j = 0; j < FIELD_WIDTH; j++) {
      if (cell[i][j].isType() == "Hero") {
        if (
          derection == "arrowup" &&
          i - 1 != -1 &&
          cell[i - 1][j].isType() != "Wall" &&
          cell[i - 1][j].isType() != "Enemy"
        ) {
          ...
          derection == "arrowleft" &&
          j - 1 != -1 &&
          cell[i][j - 1].isType() != "Wall" &&
          cell[i][j - 1].isType() != "Enemy"
        ) {
...
function spawnTile() {
  ...
  for (let i = 0; i < 10; i++)
    field.getRandomEmptyCell().linkTile(new Heal(gameBoard));
  for (let i = 0; i < 2; i++)
    field.getRandomEmptyCell().linkTile(new Sword(gameBoard));
  for (let i = 0; i < 10; i++)
    field.getRandomEmptyCell().linkTile(new Enemy(gameBoard));
}
////
